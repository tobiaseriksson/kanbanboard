    ...
    chart.label = function (labels, isBottom) {
        ...
        if (opts.stacked) {
			...
			for( var q=0; q < bars.length; q++ ) {
				for( var w=0; w < bars[q].length; w++ ) {
					// console.log("bars["+q+"]["+w+"]="+bars[q][w]);	
					// console.log("bars["+q+"]["+w+"]x="+bars[q][w].x);						
					// console.log("bars["+q+"]["+w+"]y="+bars[q][w].y);			
				}
			}
            for (var i = 0; i < len; i++) {
                for (var j = 0; j < (multi || 1); j++) {s
                    var label = paper.g.labelise(multi ? labels[j] && labels[j][i] : labels[i], multi ? values[j][i] : values[i], total);
					var lx = 0;
					var ly = 0;			
					if( multi == 0 ) {
						lx = bars[ i ].x;
						ly = isBottom ? y + height - barvgutter / 2 : bars[ i ].y - 10;
					}
					else {						
						lx = bars[j][i].x;
						ly = isBottom ? y + height - barvgutter / 2 : bars[j][i].y - 10;
					}
					var ci = (i * (multi || 1) + j);
					// console.log("i="+i+",j="+j+"label="+label+",cover-index="+ci+",lx="+lx+",ly="+ly);
					L = paper.g.text( lx, ly, label+' ' ).insertBefore(covers[ci]);
					L = paper.g.text( lx, ly, label).insertBefore(covers[ci]);
